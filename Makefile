...
GHCR_USER ?= evanlefevre

.PHONY: help tools-check tools-check-tp3 cluster-up cluster-down argocd-install argocd-password \
        hosts-print helm-lint images-build images-load clean

help:
	@echo "Cibles disponibles :"
	@echo "  tools-check       - vérifie les CLI requises (TP 2 + TP 3)"
	@echo "  tools-check-tp3   - vérifie présence ET version minimale (TP 2 + TP 3)"
...
# Défi bonus étape 0 : vérifie présence ET version minimale de chaque outil.
# Sort en erreur explicite (exit 1) si un outil manque ou est trop ancien.
tools-check-tp3:
	@echo ">>> Vérification outillage TP 2 + TP 3 (présence + version minimale)"
	@fail=0; \
	verlte() { [ "$$1" = "$$(printf '%s\n%s' "$$1" "$$2" | sort -V | head -n1)" ]; }; \
	check() { \
	  name="$$1"; min="$$2"; cur="$$3"; \
	  if [ -z "$$cur" ]; then echo "  ✗ $$name : ABSENT (>= $$min requis)"; fail=1; return; fi; \
	  if verlte "$$min" "$$cur"; then echo "  ✓ $$name : $$cur (>= $$min)"; \
	  else echo "  ✗ $$name : $$cur < $$min requis"; fail=1; fi; \
	}; \
	check docker   20.10  "$$(docker version --format '{{.Client.Version}}' 2>/dev/null | tr -d 'v')"; \
	check kubectl  1.28   "$$(kubectl version --client -o json 2>/dev/null | sed -n 's/.*gitVersion\": *\"v\([0-9.]*\).*/\1/p' | head -n1)"; \
	check helm     3.12   "$$(helm version --short 2>/dev/null | sed 's/^v//; s/+.*//')"; \
	check kind     0.22   "$$(kind version 2>/dev/null | sed -n 's/.*kind v\([0-9.]*\).*/\1/p')"; \
	check argocd   2.10   "$$(argocd version --client --short 2>/dev/null | sed -n 's/.*v\([0-9.]*\).*/\1/p' | head -n1)"; \
	check promtool 2.50   "$$(promtool --version 2>&1 | sed -n 's/.*version \([0-9.]*\).*/\1/p' | head -n1)"; \
	check jq       1.7    "$$(jq --version 2>/dev/null | sed 's/jq-//')"; \
	rollouts="$$(kubectl argo rollouts version 2>/dev/null | sed -n 's/.*: *v\([0-9.]*\).*/\1/p' | head -n1)"; \
	check kubectl-argo-rollouts 1.7 "$$rollouts"; \
	if [ $$fail -ne 0 ]; then echo ">>> Outillage incomplet."; exit 1; fi; \
	echo ">>> Tous les outils sont présents et à jour."
